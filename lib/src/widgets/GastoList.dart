...
class GastoList extends StatefulWidget {
  ...
  final GastoListBloc listaBloc;

  GastoList({ this.items, this.listaBloc }) : super();
  @override
  _GastoListState createState() => _GastoListState();
}

class _GastoListState extends State<GastoList> {
  ModelLista listaBo = ModelLista();
  ModelGasto itemBo = ModelGasto();
            ...
            onSelected: (ListAction result){
              switch(result){
                ...
                case ListAction.delete:
                  itemBo.delete(item['pk_gasto']).then((deleted){
                    if(deleted){
            ...
            RaisedButton(
              ...
              onPressed: (){
                ...
                listaBo.update({
                  ...
                  'valor': _vEdit.text,
                  'criado': DateTime.now().toString()
                }, item['pk_gasto']).then((saved){
                  Navigator.of(ctx).pop();
                  Navigator.of(ctx).pushReplacementNamed('/gastos');
